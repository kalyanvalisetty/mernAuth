...
const Login = () => {
  ...
  const onSubmitHandler = async(e)=>{
    try{
      ...
      if(state == 'Sign Up'){
        const {data} = await axios.post(backendUrl+'/api/auth/register',{userName: name, emailId: email, password: password})
        if(data.success){
          setIsLoggedin(true)
          getUserData()
          navigate('/')
        ...
        const {data} =  await axios.post(backendUrl+'/api/auth/login',{emailId: email, password: password})
        if(data.success){
          setIsLoggedin(true)
          getUserData()
          navigate('/')
  ...
  return (
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
      ...
      <p className="text-sm text-center mb-6">{state=='Sign Up'? 'Create your account': 'Login into your account!'}</p>
      
      <form onSubmit={onSubmitHandler}>
        {state=='Sign Up' && (<div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C] '>
          <img src={assets.person_icon} alt="person" />
          <input onChange={(e)=>setName(e.target.value)} value={name} className='bg-transparent outline-none' type="text" placeholder="User Name" required/>
        </div>)}
